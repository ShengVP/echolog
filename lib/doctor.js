...
const VAULT_DIR = path.join(PROJECT_DIR, 'Daily_Vault');
...
function checkDaemon() {
  ...
  if (!fs.existsSync(pidFile)) {
    console.log(fail('未运行（找不到 PID 文件）'));
    console.log(dim('  → 运行 echolog start 启动'));
    return false;
  }
  const pid = parseInt(fs.readFileSync(pidFile, 'utf8'), 10);
  try {
    process.kill(pid, 0); // 0 = 仅检查存在
    const mem = execSync(`ps -o rss= -p ${pid}`).toString().trim();
    const memMB = (parseInt(mem, 10) / 1024).toFixed(1);
    console.log(ok(`运行中 pid=${pid}, mem=${memMB} MB`));
    return true;
  } catch {
    console.log(fail(`PID ${pid} 不存在（可能崩溃了，PID 文件残留）`));
    console.log(dim('  → 运行 echolog start 重启'));
    return false;
  }
}
...
function checkLocalDeps() {
  ...
  try {
    ...
    console.log(ok('whisper-cli 已安装'));
  } catch {
    console.log(warn('whisper-cli 未安装（语音转录需要）—— brew install whisper-cpp'));
  }
  // whisper model
  const modelPath = path.join(process.env.HOME, '.whisper-models', 'ggml-large-v3-turbo-q5_0.bin');
  ...
  try {
    ...
    console.log(ok('ffmpeg 已安装'));
  } catch {
    console.log(fail('ffmpeg 缺失 —— brew install ffmpeg'));
  }
