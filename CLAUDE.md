...
# 不用 CLI 时直接跑（前台）
npm run feishu           # = node index-7-feishu.js
npm run tg               # = node index-6-tg.js
```

CLI 实现：`bin/echolog` (bash 脚本)，PID 写到 `~/.echolog/<channel>.pid`，日志按天写到 `~/.echolog/logs/<channel>-YYYY-MM-DD.log`。改 CLI 行为时只动 `bin/echolog`，不要重写成 node 脚本（保持轻量、无依赖）。
...
飞书通道还需要：
- `brew install whisper-cpp ffmpeg` —— 语音转文字
...
只有两个入口，互不依赖：
- `index-7-feishu.js` —— 飞书通道（主推，无需代理，支持图片/视频/语音/ASR/markdown 卡片渲染/离线追写）
- `index-6-tg.js` —— Telegram 通道（备用，需要本地代理 127.0.0.1:7897）
...
不要把任何 secret 写回源码（`index-6-tg.js` 早期版本写过，已迁移）。
...
- **TG 白名单**：`index-6-tg.js` 的 `bot.use` 里 `ctx.from?.id !== MY_TELEGRAM_ID` 直接 return，连 `next()` 都不调，所有非本人消息静默丢弃。改动这个 middleware 等于打开后门。
- **飞书首次配对锁**：`index-7-feishu.js` 第一次收到 p2p 消息把 `sender.open_id` 写进 `.feishu_state.json`；之后任何不匹配的 sender 在 `handleMessage` 顶部就被丢弃。bot 是私有应用、只有你自己能 DM，所以「首条消息 = 配对」这个语义是安全的。要重置就删掉 state 文件里的 `paired_open_id`。
- **TG 硬编码 secret**：`TOKEN` 和 `MY_TELEGRAM_ID` 直接写在 `index-6-tg.js` 头部，是已知代码味道；仓库私有不公开推送，trade-off 接受。如果改 env 变量，先 `git log -p index-6-tg.js` 核对历史。
