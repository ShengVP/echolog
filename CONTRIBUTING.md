...
# or, without the CLI:
npm run feishu           # = node feishu.js  (foreground)
npm run tg               # = node telegram.js       (Telegram, needs a local proxy)
```
...
```
feishu.js     Feishu channel entry (recommended — images/video/voice/ASR/cards/backfill)
telegram.js         Telegram channel entry (fallback — needs a local proxy)
...
# 不用 CLI 时直接跑：
npm run feishu           # = node feishu.js（前台）
npm run tg               # = node telegram.js（Telegram，需本地代理）
```
...
```
feishu.js     飞书通道入口（主推 —— 图片/视频/语音/ASR/卡片/离线追写）
telegram.js         Telegram 通道入口（备用 —— 需本地代理）
